...
Заливаем репозиторий в домашний каталог сайта, **но корнем сайта делаем
`public/`** — остальное не должно быть доступно из веба:

```bash
cd /var/www/ВАШ_ПОЛЬЗОВАТЕЛЬ/data
git clone https://github.com/staschern/civi.git civi
cd civi
```

Права: PHP должен уметь писать только в каталог загрузок.

```bash
chown -R ВАШ_ПОЛЬЗОВАТЕЛЬ:ВАШ_ПОЛЬЗОВАТЕЛЬ /var/www/ВАШ_ПОЛЬЗОВАТЕЛЬ/data/civi
chmod -R 755 /var/www/ВАШ_ПОЛЬЗОВАТЕЛЬ/data/civi
chmod -R 775 /var/www/ВАШ_ПОЛЬЗОВАТЕЛЬ/data/civi/public/uploads
```
...
### Через панель управления (ISPmanager и подобные)
...
Откройте сайт — должна открыться форма входа. После входа:
...
| Картинки технологий не загружаются | нет прав на запись в `public/uploads/tech` |
