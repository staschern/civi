...
(по умолчанию 60). Дальше работают два коэффициента: следующий столбец
внутри эпохи дороже предыдущего в `cost_step_lane` раз (по умолчанию 1.1),
а первый столбец новой эпохи — в `cost_step_era` раз (по умолчанию 1.5).
Внутри эпохи цена растёт полого, на смене эпохи делает заметный шаг.

Внутри столбца стоимости расходятся случайно, но разброс считается
от меньшего из коэффициентов так, чтобы диапазоны соседних столбцов
не пересекались: чем меньше коэффициент, тем уже разброс. Все три числа
правятся прямо на доске — коэффициенты в панели над деревьями,
среднее — над каждым столбцом.
