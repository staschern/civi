...
Одна миграция, в ней и схема, и стандартный набор данных на 2026.08.12.

```bash
mysql -u root -p civi < db/migrations/0001_create_tech_tree_versions.sql
```

Файл миграции собирается скриптом и руками не правится: схема лежит
в [`schema/0001_schema.sql`](schema/0001_schema.sql), а каталог эпох, веток
и технологий вынимается прямо из данных прототипа доски — так стандартный
набор в базе не расходится с тем, что нарисовано на доске.

```bash
node tools/generate-migration.js     # правим схему или прототип — и пересобираем
```
...
все 15 эпох и все 353 позиции каталога (187 технологий + 166 соц. концепций),
